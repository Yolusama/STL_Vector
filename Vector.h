...
namespace MStd {
	...
	private:
		...
		public:
			...
			ReIterator<T>& operator --(int)
			{
				++this->Ptr;
				return *this;
			}
			ReIterator<T> operator --() {
				this->Ptr++;
				return *this;
			}
			bool operator > (ReIterator<T> iterator) {
				return this->Ptr< iterator.Ptr;
			}
			bool operator >= (ReIterator<T> iterator) {
				return this->Ptr <= iterator.Ptr;
	...
	protected:
		...
		void CopyData(T*& to, T* from, int length) {
	...
	public:
		...
		const VReIterator crbegin() const{
			return VReIterator(data + len - 1);
		}
		const VReIterator crend() const {
			return VReIterator(data - 1);
		}
		...
		VLong index_of(T val) {
			for (VLong i = 0; i < len; i++)
				if (data[i] == val)
					return i;
			return -1;
		}
		VIterator find(T val) {
			for (VIterator it = begin(); it != end(); it++)
				if (*it == val)
					return it;
			return end();
		}
		VIterator find_last(T val) {
			VIterator it = VIterator();
			for (VIterator i = begin(); i != end(); i++)
				if (*i == val)
					it = i;
			return it;
		}
